...
Please note that the folder "Publish_codes.zip" is not the library but it is only used for testing in our publication. We will develop/ integrate the codes to one unified KARL library (similar with LibSVM). In this library, we hope to support more machine learning models/ kernel functions. Moreover, we also hope to support more interesting applications using kernel functions.
